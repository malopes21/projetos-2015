...
function TabelaSimbolos() {
    ...
    TabelaSimbolos.prototype.getValor = function(token) {
        return this.simbolos[token.indice].valor;
    };
    
    TabelaSimbolos.prototype.setValor = function (token, valor) {
        this.simbolos[token.indice].valor = valor;
    };
}
...
function AnalisadorSemantico(raiz) {
    ...
    AnalisadorSemantico.prototype.analisa = function (no) {
        switch (no.tipo) {
            case "ListComan":
                return this.ListComan(no);
            case "Coman":
                return this.Coman(no);
            case "ComanInter":
                return this.ComanInter(no);
            case "Decl":
                return this.Decl(no);
            case "Tipo":
                return this.Tipo(no);
            case "ListId":
                return this.ListId(no);
            case "ListId2":
                return this.ListId2(no);
            case "Atrib":
                return this.Atrib(no);
            case "ExprArit":
                return this.ExprArit(no);
            case "Operan":
                return this.Operan(no);
            case "OpArit":
                return this.OpArit(no);
            case "Escrita":
                return this.Escrita(no);
            case "Leitura":
                return this.Leitura(no);
            case "Laco":
                return this.Laco(no);
            case "ExprLog":
                return this.ExprLog(no);
            case "OpLog":
                return this.OpLog(no);
            case "ExprRel":
                return this.ExprRel(no);
            case "OpRel":
                return this.OpRel(no);
            case "Cond":
                return this.Cond(no);
            case "Senao":
                return this.Senao(no);
            case "Token":
                break;
            default :
                throw new Error("Tipo de no nao existente! Método AnalisadorSemantico.analisa(no). No: " + no.toString());
        }
    };
    ...
    AnalisadorSemantico.prototype.ExprArit = function (no) {
        if (no.filhos.length === 1) {
            ...
            if (opArit.imagem === ".") {
                return exprArit1.concat(exprArit2);  //junta 2 arrays ??
            } else {
    ...
    AnalisadorSemantico.prototype.Laco = function (no) {
        this.analisa(no.get(1));
        this.analisa(no.get(2));
    };

    //<Expr_Log> ::= '(' <Op_Log> <Expr_Log> <Expr_Log> ')' | <Expr_Rel>
    AnalisadorSemantico.prototype.ExprLog = function (no) {
        if (no.filhos.length === 1) {
            return this.analisa(no.get(0));
        } else {
            var exprLog1 = this.analisa(no.get(2));
            var exprLog2 = this.analisa(no.get(3));

            var tudo = exprLog1.concat(exprLog2);
            var operan0 = tudo[0];
            var tipoOperan0 = tabelaSimbolos.getTipo(operan0);
 
            for (var i = 1; i < tudo.length; i++) {
                var tipoOperan = tabelaSimbolos.getTipo(tudo[i]);
                if (tipoOperan !== tipoOperan0) {
                    this.erros.push("Tipo de operando incompatível na expressao. Linha: " + operan0.linha + ". Token: " + operan0.toString());
                }
            }
            return tudo;
        }
...
function Interpretador(raiz) {
    this.raiz = raiz;

    Interpretador.prototype.interpretar = function () {
        this.interpreta(raiz);
    };
    
    Interpretador.prototype.interpreta = function (no) {
        switch (no.tipo) {
            case "ListComan":
                return this.ListComan(no);
            case "Coman":
            ...
            case "Token":
                break;
            default :
                throw new Error("Tipo de no nao existente! Método Interpretador.intgerpreta(no). No: " + no.toString());
        }
    };
    
        //<List_Coman> ::= <Coman> <List_Coman> |
    Interpretador.prototype.ListComan = function (no) {
        if (no.filhos.length > 0) {
            this.interpreta(no.get(0));
            this.interpreta(no.get(1));
        }
    };

    //<Coman> ::= '(' <Coman_Inter> ')'
    Interpretador.prototype.Coman = function (no) {
        this.interpreta(no.get(1));
    };

    //<Coman_Inter> ::= <Atrib> | <Decl> | <Escrita> | <Leitura> | <Laco> | <Cond> | <List_Coman>
    Interpretador.prototype.ComanInter = function (no) {
        this.interpreta(no.get(0));
    };

    //<Decl> ::= <Tipo> <List_Id>
    Interpretador.prototype.Decl = function (no) {
        //nada aqui!
    };

    //<Tipo> ::= 'intero' | 'vero' | 'corda' | 'booleano'
    Interpretador.prototype.Tipo = function (no) {
        //nada aqui!
    };

    //<List_Id> ::= id <List_Id2>
    Interpretador.prototype.ListId = function (no) {
        //nada aqui!
    };

    //<List_Id2> ::= id <List_Id2> |
    Interpretador.prototype.ListId2 = function (no) {
        //nada aqui!
    };

    //será usado para converter o lado direito da expressao de atribuicao
    //var tipoLeftId = "";

    //<Atrib> ::= '=' id <Expr_Arit> 
    Interpretador.prototype.Atrib = function (no) {
        var id = no.get(1).token;
        //tipoLeftId = tabelaSimbolos.getTipo(id);
        var exprArit = this.interpreta(no.get(2)); 
        tabelaSimbolos.setValor(id, exprArit);
    };

    //<Expr_Arit> ::= <Operan> | '(' <Op_Arit> <Expr_Arit> <Expr_Arit> ')'
    Interpretador.prototype.ExprArit = function (no) {
        if(no.filhos.length === 1) {
            return this.interpreta(no.get(0));
        } else {
            var expr1 = this.interpreta(no.get(2));
            var expr2 = this.interpreta(no.get(3));
            var op = this.interpreta(no.get(1));
            if(op === "+") {
                return expr1 + expr2;
            } else if(op === "-") {
                return expr1 - expr2;
    ...
    //<Operan> ::= id | cli | cls | clr
    Interpretador.prototype.Operan = function (no) {
        var operan = no.get(0).token;
        if(operan.classe === "ID" ) {
            return tabelaSimbolos.getValor(operan);
        } else if(operan.classe === "CLI") {
            return parseInt(operan.imagem);
        } else if(operan.classe === "CLR") {
            return parseFloat(operan.imagem);
        } else if(operan.classe === "CLS") {
            return operan.imagem;
        } 
    };

    //<Op_Arit> ::= '+' | '-' | '*' | '/' | '.'
    Interpretador.prototype.OpArit = function (no) {
        return no.get(0).token.imagem;
    };

    //<Escrita> ::= 'stampa' <Expr_Arit>
    Interpretador.prototype.Escrita = function (no) {
        var exprArit = this.interpreta(no.get(1));
        var console = document.getElementById("console");
        //console.value = console.value + exprArit.toString();
        var dados = exprArit.toString();
        //var barra = false;
        for(var i=0; i<dados.length; i++) {
            if(dados[i] === '\\') {
                console.value = console.value + "\n";
            } else {
                console.value = console.value + dados[i];
            }
        }
    };

    //<Leitura> ::= 'entrare' id
    Interpretador.prototype.Leitura = function (no) {
        var id = no.get(1).token;
        var tipoId = tabelaSimbolos.getTipo(id);
        var valorDigitado = window.prompt("Entre valor para '"+id.imagem+"': ");
        var console = document.getElementById("console");
        console.value = console.value + valorDigitado;
        if(tipoId === "intero") {
            tabelaSimbolos.setValor(id, parseInt(valorDigitado));
        } else if(tipoId === "vero") {
            tabelaSimbolos.setValor(id, parseFloat(valorDigitado));
        } else if(tipoId === "corda") {
            tabelaSimbolos.setValor(id, valorDigitado);
        }
    };

    //<Laco> ::= 'mentre' <Expr_Log> <List_Coman>
    Interpretador.prototype.Laco = function (no) {
        var condicao = this.interpreta(no.get(1));
        while(condicao) {
            this.interpreta(no.get(2));
            condicao = this.interpreta(no.get(1));
        }
    };

    //<Expr_Log> ::= '(' <Op_Log> <Expr_Log> <Expr_Log> ')' | <Expr_Rel>
    Interpretador.prototype.ExprLog = function (no) {
        if (no.filhos.length === 1) {
            return this.interpreta(no.get(0));
        } else {
            var exprLog1 = this.interpreta(no.get(2));
            var exprLog2 = this.interpreta(no.get(3));

            var op = this.interpreta(no.get(1));
            if(op === "&&") {
                return exprLog1 && exprLog2;
            } else if(op === "||") {
                return exprLog1 || exprLog2;
            } 
        }
    ...
    //<Op_Log> ::= '&&' | '||'
    Interpretador.prototype.OpLog = function (no) {
        return no.get(0).token.imagem;
    };

    //<Expr_Rel> ::= '(' <Op_Rel> <Expr_Arit> <Expr_Arit> ')'
    Interpretador.prototype.ExprRel = function (no) {
        var exprArit1 = this.interpreta(no.get(2));
        var exprArit2 = this.interpreta(no.get(3));
        
        var op = this.interpreta(no.get(1));
        if(op === ">") {
            return exprArit1 > exprArit2;
        } else if(op === ">=") {
            return exprArit1 >= exprArit2;
        } else if(op === "<") {
            return exprArit1 < exprArit2;
    ...
    //<Op_Rel> ::= '>' | '<' | '<=' | '>=' | '==' | '!='
    Interpretador.prototype.OpRel = function (no) {
        return no.get(0).token.imagem;
    };

    //<Cond> ::= 'si' <Expr_Log> <Coman> <Senao>
    Interpretador.prototype.Cond = function (no) {
        var condicao = this.interpreta(no.get(1));
        if(condicao) {
            this.interpreta(no.get(2));
        } else {
            this.interpreta(no.get(3));
        }
    };

    //<Senao> ::= <Coman> |
    Interpretador.prototype.Senao = function (no) {
        if(no.filhos.length > 0) {
            this.interpreta(no.get(0));
        }
    };
...
function main() {
    ...
    document.getElementById("console").value = "";      //limpar a 'console'
    ...
    analisadorLexico.mostraSimbolos(); //mostrar os simbolos para verificacao da tipagem
    
    var interpretador = new Interpretador(analisadorSintatico.raiz);
